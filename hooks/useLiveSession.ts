...
export const useLiveSession = (videoRef: React.RefObject<HTMLVideoElement>) => {
  ...
  const connect = useCallback(async (nickname: string, retryCount = 0) => {
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (audioData && outputCtx) {
              ...
              try {
                ...
                source.onended = () => {
                    if (outputCtx && outputCtx.currentTime >= nextStartTimeRef.current - 0.1) setIsSpeaking(false);
                }
              } catch (decodeErr) {}
            }
          },
          ...
          onerror: (err) => {
            ...
            if (!isIntentionalStopRef.current && retryCount < 5) {
                ...
                retryTimeoutRef.current = window.setTimeout(() => connect(nickname, retryCount + 1), delay);
            }
          }
        }
      });
        ...
        canvas.toBlob(async (blob) => {
            if (blob) {
                try {
                    ...
                    sessionPromise.then(session => {
                        try { session.sendRealtimeInput({ media: { mimeType: 'image/jpeg', data: base64Data } }); } catch (e) {}
                    }).catch(() => {});
                } catch(e) {}
            }
        }, 'image/jpeg', 0.5); 
  ...
  return { connect, disconnect, isConnected, isSpeaking, audioVolume };
};